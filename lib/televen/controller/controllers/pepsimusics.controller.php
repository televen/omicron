...
class pepsimusics{
	...
	public static function PPM_360($args){
		return array("number" => $args[0]['wich']);
	}
	
	public static function tendency($args){
	}
	
	public static function biography($args){
	}
	
	public static function best_performance($args){
	}
	
	public static function trivias($args){
	}
	...
	public static function tweet_cloud($args){
		...
		foreach($info["tweets"] as $tweet){
			$position	 = pepsimusics::setRandomPosition();
			$color		 = pepsimusics::setColor($position);
	...
	public static function best_dressed($args){
		$files = array();
		$return = array("photos" => "", "intro" => "");
		$path = $GLOBALS['base_URI'] . $GLOBALS['show_URI'] . "pepsimusics/assets/best/";
		...
		shuffle($files);
		$i = -1; $indexes = count($files) - 1; $count = 1;
		$html = "";
		
		$folder = $path;
		$html = $html . '<li data-images="rm_container_1" data-rotation="-15"><img src="' . $folder . $files[$i+1]. '"  data-id="'. $files[$i+1]. '"/></li>';
		$html = $html . '<li data-images="rm_container_2" data-rotation="-5"><img src="' . $folder . $files[$i+2]. '" data-id="'. $files[$i+2]. '"/></li>';
		$html = $html . '<li data-images="rm_container_3" data-rotation="5"><img src="' . $folder . $files[$i+3]. '" data-id="'. $files[$i+3]. '"/></li>';
		$html = $html . '<li data-images="rm_container_4" data-rotation="15"><img src="' . $folder . $files[$i+4]. '" data-id="'. $files[$i+4]. '"/></li>';
		$i = $i + 4;
		$return["intro"] = $html; 
		
		$html = "";
		while($i < $indexes){
			$html = $html . '<div id="rm_container_' . $count . '">';
			$html = $html . '<img src="' . $folder . $files[$i+1]. '" data-id="'. $files[$i+1]. '"/>';
			$html = $html . '<img src="' . $folder . $files[$i+2]. '" data-id="'. $files[$i+2]. '"/>';
			$html = $html . '<img src="' . $folder . $files[$i+3]. '" data-id="'. $files[$i+3]. '"/>';
			$html = $html . '<img src="' . $folder . $files[$i+4]. '" data-id="'. $files[$i+4]. '"/>';
			$html = $html . '</div>';
			$i = $i + 4;
			$count = $count + 1;
		}
