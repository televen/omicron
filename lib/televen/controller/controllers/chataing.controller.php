...
include $GLOBALS["root"] . "lib/televen/model/chataing.model.php";

class chataing{
	public static function stop_motion($args){
		$model = new chataing_model();
		$folder = $model->getStopMotionFoldersByBlack($args[0]['black']);
		print_r($folder);
	}
	public static function photo_gallery($args){
		...
		$path = $GLOBALS['base_URI'] . $GLOBALS['show_URI'] . "chataing/assets/" . $folder;
